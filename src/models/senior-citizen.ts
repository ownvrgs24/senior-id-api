    ...
    const results = await Promise.all(
      data.map(async (item) => {
        return await prismaDatabase.senior_citizen_details.upsert({
          ...
          update: {
            first_name: decodeURI(encodeURI(item.first_name)),
            middle_name: decodeURI(encodeURI(item.middle_name)) || null,
            last_name: decodeURI(encodeURI(item.last_name)),
            date_of_birth: item.date_of_birth ? new Date(item.date_of_birth) : new Date(),
            place_of_birth: decodeURI(encodeURI(item.place_of_birth)),
            full_address: decodeURI(encodeURI(item.full_address)),
            civil_status: decodeURI(encodeURI(item.civil_status)) || null,
            contact_number: decodeURI(encodeURI(item.contact_number)),
            email: decodeURI(encodeURI(item.email)) || null,
